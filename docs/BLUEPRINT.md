...
- ~~**`port_in_use()` is a bare 300ms TCP connect** that never probes `/api/health`. Any unrelated listener on 8756 is silently adopted as the Job Hunter API.~~ **Fixed in TASK 006, and reproduced before and after.** A decoy server answering 200 with foreign JSON on 8756 was in fact adopted, and every subsequent request 404'd with no error path. Behind it sat a second defect the reproduction exposed: the shell's readiness check was a TCP connect and the renderer's own `fetch` had no timeout, so a socket that *accepted a connection and then stayed silent* was indistinguishable from a healthy backend and cost the full startup timeout to discover. That half was fixed in `5537bdb`; the port half is fixed here. The shell now sends `GET /api/health` with a bounded connect and a bounded read, and requires the response to carry `"service": "job-hunter"` — a literal agreed on both sides, not `app_name`, so the handshake survives a rename. A 200 is not enough. A port that answers as anything else is named in the log with the reason and the search moves on; a port nobody holds is confirmed bindable; and a range with nothing free is `StartError::PortUnavailable`, which names the port and tells the user to close what is holding it, rather than the old `free_port()` behaviour of silently returning the starting port for `uvicorn` to fail to bind.
- ~~**The API address is injected by `window.eval` at `PageLoadEvent::Started`**, while `const BASE = resolveBase()` runs at module-eval time. The ordering is not guaranteed; the correct mechanism is `initialization_script()`. Whenever the port is reassigned, every request goes to the wrong place with no diagnostic. This also forces `'unsafe-inline'` in `script-src`.~~ **Fixed in TASK 006.** The window carries `"create": false` in `tauri.conf.json` and is rebuilt from that same config in `setup()` with `initialization_script()`, which Tauri guarantees runs before any page script. The port is therefore decided before the window exists. `'unsafe-inline'` is gone from `script-src`, and both build paths were run with it gone.
